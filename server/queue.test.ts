...
describe("Queue Router - Move to Queue", () => {
  ...
  it("should move an approved draft to queue", async () => {
    ...
    const result = await caller.moveToQueue({
      brandId: testBrandId,
      draftId: testDraftId,
    });
  ...
  it("should create a post entry in the queue", async () => {
    ...
    // Move draft to queue
    await caller.moveToQueue({
      brandId: testBrandId,
      draftId: testDraftId,
    });

    // Verify post was created
    const queuedPosts = await caller.getQueue({
      brandId: testBrandId,
    });
  ...
  it("should move draft with multiple platforms to queue", async () => {
    const testDraftId2 = "test-draft-queue-002";

    // Create another test draft
    await db
      .insert(drafts)
      .values({
        id: testDraftId2,
    ...
    expect(result.platforms).toEqual(["instagram", "facebook", "linkedin"]);
  ...
  it("should schedule posts with 3-hour gaps", async () => {
    ...
    // Move with multiple platforms to test scheduling gaps
    const result = await caller.moveToQueue({
      brandId: testBrandId,
    ...
    if (movedPosts.length === 2) {
      const time1 = new Date(movedPosts[0].scheduledFor).getTime();
      const time2 = new Date(movedPosts[1].scheduledFor).getTime();
      const hourDifference = (time2 - time1) / (1000 * 60 * 60);
      expect(hourDifference).toBeCloseTo(3, 0);
    }
